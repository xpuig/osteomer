    ...
    <section class="min-h-screen bg-osteomer-gray">
      <div class="mx-auto max-w-6xl px-6 lg:px-8">
        <div class="py-16">
          ...
          <div *ngIf="!auth.isLoggedIn" class="mx-auto max-w-md pt-20">
            <h1 class="mb-2 text-center text-2xl font-bold tracking-[-0.02em] text-black">Acceso restringido</h1>
            <p class="mb-8 text-center text-sm text-black/55">Inicia sesión para gestionar las citas</p>
            <div class="rounded border border-black/20 bg-white p-8">
              <input
                [(ngModel)]="loginEmail"
                type="email"
                placeholder="Email"
                class="mb-4 w-full rounded border border-black/20 px-4 py-3 text-base text-black outline-none transition-colors focus:border-black"
              />
              <input
                ...
                class="mb-4 w-full rounded border border-black/20 px-4 py-3 text-base text-black outline-none transition-colors focus:border-black"
              />
              <p *ngIf="loginError" class="mb-4 text-sm text-black/55">{{ loginError }}</p>
              <button
                (click)="doLogin()"
                [disabled]="!loginEmail || !loginPassword || loggingIn"
                class="w-full rounded bg-black px-6 py-3 text-sm font-bold text-white transition-all hover:bg-black/80 disabled:opacity-50"
              >
          ...
          <div *ngIf="auth.isLoggedIn">
            <div class="mb-10 flex items-center justify-between">
              <div>
                <h1 class="text-3xl font-bold tracking-[-0.02em] text-black">Backoffice — Citas</h1>
                <p class="mt-2 text-base text-black/65">
                  {{ bookings.length }} reserva(s) &middot;
            ...
            <div *ngIf="loading" class="py-20 text-center text-black/50">Cargando citas...</div>
              ...
              <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-black/5">
                  <tr *ngFor="let b of bookings" class="transition-colors hover:bg-black/[0.02]">
                    ...
                    <td class="px-4 py-3 text-black/65"><a [href]="'mailto:' + b.email" class="underline transition-colors hover:text-black">{{ b.email }}</a></td>
                    ...
                    <td class="px-4 py-3">
                      <button (click)="deleteBooking(b)" class="text-xs text-black/40 underline transition-colors hover:text-black">Eliminar</button>
                    </td>
                  </tr>
                </tbody>
              </table>
...
export class BackofficeComponent implements OnInit {
  ...
  async doLogin(): Promise<void> {
    this.loggingIn = true;
    this.loginError = '';
    const err = await this.auth.login(this.loginEmail, this.loginPassword);
  ...
  private async loadBookings(): Promise<void> {
    this.loading = true;
    try {
      this.bookings = await this.supabase.getAll();
    } catch {}
    this.loading = false;
  }
